module Jekyll
  ...
  class CssMinifyGenerator < Generator
    ...
    def generate(site)
      ...
      $minified_css_filename = last_modified.strftime("%Y%m%d%H%M") + '.min.css'
      ...
      minify_css(files_to_minify, output_file, site.source)
  ...
  class CssMinifyLinkTag < Liquid::Tag
    ...
    def render(context)
      config = Jekyll::CssMinifyGenerator.get_config
      File.join(config['css_destination'], $minified_css_filename)
    end
  end
end
