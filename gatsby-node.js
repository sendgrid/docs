...
exports.onCreateNode = ({ node, boundActionCreators, getNode }) => {
  ...
  if (node.internal.type === 'MarkdownRemark') {
    ...
    /**
     * Check if doc is "user-interface" or "for developers" and add a field slug to represent this.
     */
    let docType;
    if (permalink.match(/user-interface\/[^/]+/)) {
      docType = 'user-interface';
  ...
  return new Promise((resolve, reject) => {
    ...
    resolve(graphql(`
      ...
      result.data.allMarkdownRemark.edges.forEach((edge) => {
        ...
        // aggregate "user-interface" categories
        if (docType === 'user-interface') {
      ...
      helpCategoryList.forEach((category, i) => {
        // Create "user-interface" category nodes.
        const cat = {
          id: `${i}`,
          slug: category,
          parent: '__SOURCE__',
        ...
        // Create "/user-interface/<category-slug>" pages.
        createPage({
          path: `/user-interface/${_.kebabCase(category)}/`,
          component: categoryPage,
          context: {
            docType: 'user-interface',
            category,
