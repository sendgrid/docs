...
This functionality can be replicated using SendGrid's [SMTP API]({{root_url}}http://localhost:8000/for-developers/sending-email/building-an-x-smtpapi-header/), using [Substitution Tags]({{root_url}}/for-developers/sending-email/substitution-tags/) and [Custom Fields]({{root_url}}/ui/managing-contacts/custom-fields/) in [Marketing Campaigns]({{root_url}}/ui/managing-contacts/create-and-manage-contacts/) for marketing emails and our [transactional templates application](https://sendgrid.api-docs.io/v3.0/transactional-templates) for transactional emails.
