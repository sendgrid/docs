...
This functionality can be replicated using SendGrid's [SMTPAPI]({{root_url}}/for-developers/sending-email/building-an-smtp-email/), using [Substitution Tags]({{root_url}}/for-developers/sending-email/substitution-tags/) and [Custom Fields]({{root_url}}/user-interface/sending-email/custom-fields/) in [Marketing Campaigns]({{root_url}}/user-interface/managing-contacts/adding-contacts/) for marketing emails and our [transactional templates application](https://sendgrid.com/docs/API_Reference/Web_API_v3/Transactional_Templates/smtpapi.html) for transactional emails.
