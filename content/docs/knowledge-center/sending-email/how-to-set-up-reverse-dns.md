...
You can only set up reverse DNS if you have a [dedicated IP]({{root_url}}/knowledge-center/account-and-settings/dedicated-ip-addresses/).
...
6. Next, you need to add all of the A Records on this screen to your DNS host. This process varies depending on your DNS host. For videos on how to add your A Records to some popular DNS service providers, check out these [videos]({{root_url}}/knowledge-center/sending-email/dns-providers/).
...
If you click verify, and only half of your A name records verify, this usually means that you need to wait a bit longer. It's also possible that you entered one of your records in incorrectly. For other troubleshooting information, see [Sender authentication troubleshooting]({{root_url}}/knowledge-center/sending-email/troubleshooting-sender-authentication/).
...
Only a parent account can set up reverse DNS for an IP address. For a subuser account to have an IP that has reverse DNS set up, the parent account needs to buy the IP, set up reverse DNS and then [assign the IP to the subuser]({{root_url}}/knowledge-center/account-and-settings/dedicated-ip-addresses/#assigning-a-dedicated-ip-address-to-a-subuser).
...
- [Troubleshooting]({{root_url}}/knowledge-center/sending-email/troubleshooting-sender-authentication/)
