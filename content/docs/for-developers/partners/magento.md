...
3. Emails are automatically tracked and tagged to provide you deliverability and engagement statistics viewable from the **SendGrid Dashboard**. You can also add other [categories]({{root_url}}/user-interface/analytics-and-reporting/categories/) for your emails in the field Categories.
...
5. Next, select the [unsubscribe group]({{root_url}}/user-interface/sending-email/unsubscribe-groups/).

    ![]({{root_url}}/images/magento_6.png)

6. If you would like to [configure categories for statistics]({{root_url}}/user-interface/analytics-and-reporting/categories/), you can configure it by setting the **Categories** field under **Statistics Settings**.
...
[Create a SendGrid account](https://sendgrid.com/partners/magento/) and [generate a new API key](https://app.sendgrid.com/settings/api_keys). For more information about API Keys, [click here]({{root_url}}/user-interface/account-and-settings/api-keys/).
