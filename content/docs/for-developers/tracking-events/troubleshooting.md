...
- You can use the **Test Your Integration** feature within the Event Notification Setting on the [Settings > Mail Settings](https://app.sendgrid.com/settings/mail_settings) page in the [SendGrid UI](https://app.sendgrid.com) to send simulated events to your callback URL (note: this will not work if you have basic authentication set in your endpoint URL). You can also send a POST from a shell using cURL - this will give you the full response your server is returning including the HTTP headers:
