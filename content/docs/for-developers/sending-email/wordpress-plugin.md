...
If you would like to use [templates]({{root_url}}/user-interface/sending-email/how-to-send-email-with-dynamic-transactional-templates/), you can set the template ID to be used in all of your emails on the settings page or you can set it individually for each email in the headers.
...
You can use [categories]({{root_url}}/user-interface/analytics-and-reporting/categories/) to help organize and track the emails sent from WordPress via the SendGrid plugin.
